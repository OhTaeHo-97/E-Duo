...
public class FAQDAO {
   ...
   private String sql_selectOne = "select * from FAQ where faq_id = ?";
   private String sql_insert = "insert into FAQ values((select nvl(max(faq_id), 0) + 1 from FAQ), ?, ?, ?)";
   private String sql_delete = "delete from FAQ where faq_id = ?";
   private String sql_update = "update FAQ set category = ?, title = ?, content = ? where faq_id = ?";
   ...
   public FAQVO selectOne(FAQVO vo) {
      ...
      try {
         ...
         pstmt.setInt(1, vo.getFaq_id());
         rs = pstmt.executeQuery();
         if(rs.next()) {
            data = new FAQVO();
            data.setCategory(rs.getString("category"));
            data.setContent(rs.getString("content"));
            data.setFaq_id(rs.getInt("faq_id"));
